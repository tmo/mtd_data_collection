...
def startTBed():
    ...
    ext_server = net.addHost('eserver', cls=Host, 
            ip='10.3.0.100', 
    ...
    # net.addNAT(name='nat_client', connect='s1').configDefault()
    # nat_client = net.getNodeByName("nat_client")
    net.build()
    ...
    return net, [server], [ext_server], [client],  [attacker]

def run_experiments(net, server, ext_server, client, attacker, home_dir, client_dir, attacker_dir):
    ...
    time.sleep(3)
    ext_server.cmd("service nginx restart")
    ext_server.cmd("service nginx reload")
    ...
    try:
        # run client script
        # nat_client.cmd('sudo python client_script.py {} &> {}/direct_logs/client_dirlog.txt &'.format(client_dir, home_dir))
...
if __name__ == '__main__':
    ...
    net, [server], [ext_server], [client],  [attacker]  = startTBed()
    run_experiments(net, server, ext_server, client, attacker,  home_dir, client_dir, attacker_dir)
