...
def startTBed():
    ...
    fake_server = net.addHost('fakeserver', cls=Host, 
            ip='10.4.0.10', 
    ...
    return net, [server], [client], [fake_client], [fake_server], [attacker]

def run_experiments(net, server, client, fake_client, fake_server, attacker, home_dir, client_dir, attacker_dir):
    ...
    fake_server.cmd("cd testbed/server_files/html/")
    fake_server.cmd("ip route add default via 10.4.0.10")
    fake_server.cmd("python3 -m http.server 80 &")
...
if __name__ == '__main__':
    ...
    net, [server], [client], [fake_client], [fake_server], [attacker]  = startTBed()
    run_experiments(net, server, client, fake_client, fake_server, attacker,  home_dir, client_dir, attacker_dir)
