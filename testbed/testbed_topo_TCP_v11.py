...
"""
v11:
Set up for testing diversity MTD. With 3 web-apps.
topology
...
def run_experiments(net, server, client, fake_client, attacker, home_dir, client_dir, attacker_dir):
    ...
    # server.cmd("python3 -m http.server 40001 &")
    # server.cmd("cd ../v2")
    ...
    wd1 = "/var/www/html2/"
    server.cmd("echo 'events { } http { server { listen " + server.IP()+":40002" + "; root " + wd1 + "; } }' > nginx-conf.conf") # Create server config file
    server.cmd("sudo nginx -c " + wd + "/nginx-conf.conf &") # Tell nginx to use configuration from the file we just created
